...
class TTTProblem(AdversarialSearchProblem[TTTState, Action]):
    ...
    def heuristic_func(self, state: TTTState, player_index: int) -> float:
        ...
        for r in range(n):
            first_to_show = None
            for c in range(n):
                if first_to_show is None and state.board[r][c] is not SPACE:
                    first_to_show = state.board[r][c]
                    if first_to_show is X:
                        o_wins -= 1
                    elif first_to_show is O:
                        x_wins -= 1
                elif first_to_show is X and state.board[r][c] is O:
                    x_wins -= 1
                    break
                elif first_to_show is O and state.board[r][c] is X:
                    o_wins -= 1
                    break
        ...
        first = None
        for i in range(n):
            if first is None and state.board[i][i] is not SPACE:
                first = state.board[i][i]
                if first is X:
                    o_wins -= 1
                else:
                    x_wins -= 1
            elif first is X and state.board[i][i] is O:
                x_wins -= 1
                break
            elif first is O and state.board[i][i] is X:
                o_wins -= 1
                break
        
        first = None
        for i in range(n):
            if first is None and state.board[i][n-i-1] is not SPACE:
                first = state.board[i][n-i-1]
                if first is X:
                    o_wins -= 1
                else:
                    x_wins -= 1
            elif first is X and state.board[i][n-i-1] is O:
                x_wins -= 1
                break
            elif first is O and state.board[i][n-i-1] is X:
                o_wins -= 1
                break
        print("X=", x_wins)
        print("O=", o_wins)
        if player_index is 0: #player x
            # print("player 1")
    ...
    def transition(self, state, action):
        ...
        # print(TTTState(board, 1 - state.ptm).board)
